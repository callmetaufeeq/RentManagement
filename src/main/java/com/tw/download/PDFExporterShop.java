...
public class PDFExporterShop {

	private List<Shop> listShop;

	public PDFExporterShop(List<Shop> listShop) {
	...
	private void writeTableData(PdfPTable table) {
		for (Shop s : listShop) {
			...
			if (s.getRentType() == "Y") {
				table.addCell("Yearly");
			} else {
				table.addCell("Monthly");

			}
			table.addCell(String.valueOf(s.getCategory().getCategoryName()));
	...
	public void export(HttpServletResponse response) throws DocumentException, IOException {
		...
		table.setWidths(new float[] { 1.0f, 2.5f, 1.0f, 1.5f,1.5f });
