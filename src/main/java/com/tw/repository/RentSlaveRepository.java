...
public interface RentSlaveRepository extends JpaRepository<RentSlave, Long>, JpaSpecificationExecutor<RentSlave> {
	...
	Double getSumPaid(@Param("shopId") Long shopId, @Param("year") String year, @Param("paymentType") String paymentType);
	...
	@Query("SELECT rs.depositAmount FROM RentSlave rs WHERE rs.shop.id = :shopId AND rs.year = :year AND rs.paymentType = :paymentType")
